...
class PredictionPipeline:
    def __init__(self):
        self.model_path = os.path.join("artifacts", "model_trainer", "GradientBoostingClassifier.joblib")
        self.encoder_dict = os.path.join("artifacts", "data_transformation", "encoders.pkl")
    ...
    def preprocess_data(self, input_data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            # Drop unused columns
            columns_to_drop = ['policy_number', 'insured_hobbies', 'insured_zip', 'incident_location']
            ...
            model_features = [
                ...
                'capital_gains', 'capital_loss', 'incident_city', 'incident_state',
    ...
    def predict(self, features: pd.DataFrame) -> dict:
        try:
            ...
            return {
                'prediction': bool(prediction[0]),  # Convert to boolean for clarity
                'probability': float(probability[0][1]),  # Probability of fraud
                'prediction_text': 'Fraud Detected' if prediction[0] == 1 else 'No Fraud Detected'
            }
